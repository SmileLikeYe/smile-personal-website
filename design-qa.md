...
- Current content system: markdown posts in `src/content/posts/*.md`, loaded through Vite raw imports and rendered with `react-markdown` + `remark-gfm`.
...
| Blank page / app shell | Pass | Hero, training graph, proof cards, Skill OS, markdown writing, and footer render. |
| Framework overlay | Pass | No Vite or React error overlay detected. |
| Horizontal overflow | Pass | Current 1440 viewport `scrollWidth=1440`; mobile `scrollWidth=390`. |
...
| Skill OS section | Pass | Four concrete operating loops render for Agent Harness, Codex Skills, Evaluation Stack, and Mobile Runtime. |
| Markdown writing | Pass | Five markdown demo posts render, GFM table support works, clicking a post updates the reader and active state on desktop and mobile. |
| Navigation anchors | Pass | Menu points to `#top`, `#skills`, `#work`, `#writing`, `#about`, and `#contact`; fixed header does not cover the Writing anchor. |
| Responsive layout | Pass | Mobile layout stacks hero, model core, training panels, proof cards, Skill OS, writing, and footer without horizontal scroll. |
...
- The writing system avoids shipping a heavy frontmatter parser in the browser; the local parser supports the current post metadata shape and keeps the bundle smaller.
